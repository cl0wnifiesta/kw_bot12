...
async def buy_product_show_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        data['message_id'] = (await callback.message.answer(f"<b>🈳Товар:</b> {product_info[2]}\n"
                                                            ...
                                                            f"<em>Введите количество товара, которое вы хотите приобрести:</em>", reply_markup=cancel_kb)).message_id
    ...
    try:
        async with state.proxy() as data:
            ...
            if not (0 < data['count'] <= sub_category_count):
                ...
                await state.finish()
                return
            await message.answer('Выберите способ оплаты:', reply_markup=await get_payment_kb(data['product'], data['count']))
        await state.finish()
    except ValueError:
        async with state.proxy() as data:
...
async def buy_product_operation(callback: types.CallbackQuery, state: FSMContext):
    ...
    amount = (int(count) * int(product_info[3])) - int(((int(count) * int(product_info[3]))/100)*user_info[3])
    ...
    await callback.message.edit_text("➖➖➖➖Чек оплаты➖➖➖➖\n"
                                     ...
                                     reply_markup=InlineKeyboardMarkup(row_width=1).add(
                                         ...
                                         InlineKeyboardButton(text="🔎Проверить оплату", callback_data=f"check_bill_{bill_id_result}_{product}_{count}_{amount}")).add(
                                         InlineKeyboardButton(text="❌Отменить оплату", callback_data=f"reject_bill_{bill_id_result}")))
    await callback.answer()
...
async def check_qiwi_payment(callback: types.CallbackQuery):
    ...
    if p2p_check.status == 'PAID':
        ...
        await bot.send_document(log_chat_id, open(sell_files['txt'], 'rb'))
